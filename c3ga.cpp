...
mv extractGrade2_dont_mangle_1(const mv &a)
{
...
mv log(const mv &R) {
	mv G2 = extractGrade2_dont_mangle_1(R);
	float _g2norm2_ = norm2_returns_scalar(G2);

	float _scalarPart_ = R.get_scalar();

	if (_g2norm2_ <= 0.0f) {
		if (_scalarPart_ < 0.0f) {
			mv tmp;
			tmp.set_e1_e2(3.1415926535897931f);
			return tmp;
		}
		else {
			return mv();
		...
		float _g2norm_ = ::sqrt(_g2norm2_);

		return gp(G2, ::atan2(_g2norm_, _scalarPart_)/_g2norm_);
	}
}
